...
void help(char *prg) {
  printf("%s %s (c) 2020 by %s %s", prg, VERSION, AUTHOR, RESOURCE);
  printf("Syntax: %s [-HFD] [-sSG] [-RPA] interface [target]", prg);
...
int main(int argc, char *argv[]) {
  ...
  while ((i = getopt(argc, argv, "fDFHRPAarsmSG")) >= 0) {
    ...
    case 'P':
      ...
      break;
    default:
      fprintf(stderr, "Error: invalid option %c", i);
      exit(-1);
    }
  }
  ...
  if (bsize == -1) {
    ...
    if (bsize < 1240 || bsize > 1460) {
      fprintf(stderr, "Error: invalid MTU on interface %s: %d", interface, thc_get_mtu(interface));
      exit(-1);
    }
  }
  ...
  if (argc - optind > 1) {
    if ((dst = thc_resolve6(argv[optind + 1])) == NULL) {
      fprintf(stderr, "Error: invalid target %s", argv[optind + 1]);
      exit(-1);
    }
  ...
  if (deanon == 0) {
    ...
    if (route_only == 0)
      i += 32;
//printf("i %d  route %d prefix %d", i, prefix_only, route_only);
  ...
  if (thc_get_own_mac(interface) == NULL) {
    fprintf(stderr, "Error: invalid interface %s", interface);
    exit(-1);
  }
  mac[0] = 0;
  mac[1] = 0x0c;
...
//printf("DBG: %d entries of %s %s", cnt, route_only == 0 ? "prefix" : "", prefix_only == 0 ? "route" : "");
//printf("j is %d, bsize %d", j, bsize);
  printf("Starting to flood network with router advertisements on %s (Press Control-C to end, a dot is printed for every 1000 packets):", interface);
  while (until != 1) {
...
//    usleep(1);
  ...
  if (deanon)
    printf("Privacy extension attack done.");
